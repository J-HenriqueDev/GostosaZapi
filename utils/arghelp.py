...
async def send(bot, ctx):
        ...
        if not perms:
            msg = "Use: {}{} {}\nAliases do comando: {}\nPerm: None\ndescrição: {}".format(ctx.prefix, ctx.command, msg, aliases, ctx.command.help)
        else:
            msg = "Use: {}{} {}\nAliases do comando: {}\nperm: {}\ndescrição: {}".format(ctx.prefix, ctx.command, msg, aliases, 
        ...
        try:
            msg += "\n\nSub comandos: {}".format(", ".join([x for x in ctx.command.all_commands if x not in ctx.command.all_commands[x].aliases]))
        except:
            pass
